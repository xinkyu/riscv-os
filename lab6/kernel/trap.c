...
void kerneltrap() {
    ...
    if (scause & (1L << 63)) { // 最高位为1，表示是中断
        ...
        // --- Lab6 核心修改 ---
        // 检查是否是来自 U-mode 的 Ecall (系统调用)
        // scause == 8 是 "Environment call from U-mode"
        
        // 我们需要检查 SSTATUS_SPP 位来确认陷阱是否来自用户态
        // SSTATUS_SPP == 0 表示来自 U-mode
        if ((r_sstatus() & SSTATUS_SPP) == 0 && scause == 8) {
            
            // 是系统调用
            
            // 陷阱返回地址 sepc 应该指向 ecall 之后的指令
            // 我们需要增加 4 (ecall 指令的长度)
            w_sepc(sepc + 4);

            // *** 修正点：调用 syscall() ***
            syscall();
            
            // 注意：当有真正的用户态进程后，这里不应该返回
            // 而是应该调用 usertrapret() 来返回用户空间
