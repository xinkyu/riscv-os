...
void kerneltrap(uint64 sp_val) {
    ...
    if (scause & (1L << 63)) {
        ...
        else if (cause == 9) { // [新增] Supervisor External Interrupt (VirtIO)
            virtio_disk_intr();
        }
        else {
             // 未知中断，打印一下防止静默失败
...
void trap_init(void) {
    w_stvec((uint64)kernelvec);
    // [修改] 开启 Timer (STIE) 和 External (SEIE) 中断
    w_sie(r_sie() | SIE_STIE | SIE_SEIE); 
    printf("trap_init: interrupts enabled.\n");
}